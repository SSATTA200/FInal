...
void Shader::Start(const std::string& vertFile, const std::string& fragFile)
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	shaders[0] = CreatingShader(LoadingShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreatingShader(LoadingShader(fragFile), GL_FRAGMENT_SHADER); // create fragment shader
	...
	CheckingShaderError(program, GL_LINK_STATUS, true, "Error: Shader program linking failed"); // cheack for error

	glValidateProgram(program); //check the entire program is valid
	CheckingShaderError(program, GL_VALIDATE_STATUS, true, "Error: Shader program not valid");
...
GLuint Shader::CreatingShader(const std::string& text, unsigned int type)
{
	...
	CheckingShaderError(shader, GL_COMPILE_STATUS, false, "Error compiling shader!"); //check for compile error
